...
function settle(room, entries, winnerIds, tiebreakDraws) {
  ...
  if (winnerIsBitch) {
    // Bitch wins: lone IN-player replaces pot immediately.
    // Original pot AND replacement both carry to next round (pot doubles).
    // Folders earn a free-play credit. Room locks.
    ...
    if (playerBeatBitch) {
      // Fresh-start rule: game resets cleanly, room unlocks, folders still get credit.
      room.rolloverOwed = {};
      room.carryPot = 0;
      ...
      // Multi-player showdown: each losing IN-player replaces FULL POT AMOUNT from their stack.
      // Replacement chips carry to next round. Folders earn free-play credit.
