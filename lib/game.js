...
function startRound(room) {
  ...
  // If carryPot > 0, the pot is already funded — no antes collected.
  // Antes only collected when carryPot is empty (first hand or after beat-Bitch).
...
function settle(room, entries, winnerIds, tiebreakDraws) {
  ...
  if (winnerIsBitch) {
    ...
    if (playerBeatBitch) {
      // FRESH START: game resets cleanly, room unlocks, no credits carry over.
      // Everyone antes $1 fresh next round.
      room.rolloverOwed = {};
      room.carryPot = 0;
