...
function startRound(room) {
  ...
  // ANTE COLLECTION RULE:
  // If the game is already in play (carryPot > 0), the pot is already funded
  // by the previous hand's losers — no antes are collected. This is the normal
  // running state of the game.
  // Antes are collected ONLY when carryPot is empty, which happens in two cases:
  //   1. The very first hand of a room
  //   2. After a player beats the Bitch (fresh-start reset zeros the carry)
  const collectAntes = (room.carryPot || 0) === 0;
